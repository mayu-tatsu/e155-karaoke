...
// 1.536 MHz down to a 16 kHz PCM. Acts as the master, shifting out
// 16-bit PCM samples via SPI to the MCU.

module karaoke_top (
    input  logic        pdm_data,       // from the microphone
    input  logic        reset_n,        // active-low reset from board

    output logic        clk,            // drives microphone, 1.536 MHz

    output logic        sck,            // to MCU (SPI SCK)
    output logic        sdo,           	// to MCU (SPI MOSI)
	output logic		cs              // to MCU (SPI CS)
);
